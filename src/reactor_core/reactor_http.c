...
void reactor_http_tcp_event(void *state, int type, void *data)
{
  ...
  switch (type)
    ...
    case REACTOR_TCP_ACCEPT:
      ...
      reactor_stream_init(&session->stream, reactor_http_session_request_event, session);
...
static inline void reactor_http_session_request_update(reactor_http_session *session, off_t offset)
{
...
void reactor_http_session_new(reactor_http_session **session, reactor_http *http)
{
  *session = malloc(sizeof **session);
  if (!*session)
    return;
  **session = (reactor_http_session) {.state = REACTOR_HTTP_SESSION_OPEN, .http = http};
  http->ref ++;
}
...
void reactor_http_session_request_event(void *state, int type, void *data)
{
  ...
  switch (type)
    ...
    case REACTOR_STREAM_READ:
      reactor_http_session_request_read(session, read);
      break;
...
void reactor_http_session_request_read(reactor_http_session *session, reactor_stream_data *data)
{
  size_t size;
  ...
  while (data->size)
    {
      if (!session->message_base)
        {
          reactor_http_session_request_parse(session, data);
      ...
      if (data->base != session->message_base)
        reactor_http_session_request_update(session, (char *) data->base - (char *) session->message_base);
...
void reactor_http_session_request_parse(reactor_http_session *session, reactor_stream_data *data)
{
